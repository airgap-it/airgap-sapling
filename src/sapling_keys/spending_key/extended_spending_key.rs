...
mod test {
    ...
    fn generates_extended_spending_key_from_mnemonic() {
        ...
        let _bytes = get_extended_spending_key_bytes(seed.as_bytes(), DERIVATION_PATH).unwrap();
    ...
    fn fails_to_generate_extended_spending_key_when_derivation_path_empty() {
        ...
        let error = get_extended_spending_key_bytes(seed.as_bytes(), "");
    ...
    fn fails_to_generate_extended_spending_key_when_derivation_path_missing_prefix() {
        ...
        let error = get_extended_spending_key_bytes(seed.as_bytes(), "/44'/123'/0'/0/0");
    ...
    fn fails_to_generate_extended_spending_key_when_derivation_path_invalid() {
        ...
        let invalid_character = get_extended_spending_key_bytes(seed.as_bytes(), "m/44'/123q/0'/0/0");
        let empty_index = get_extended_spending_key_bytes(seed.as_bytes(), "m/44'//0'/0/0");

        assert_eq!(invalid_character, Err(SpendingKeyError::caused_by(DerivationPathError::invalid_character(vec!["q"]).to_string())));
