...
use crate::common::utils::c_utils::{c_copy_result_res, c_deserialize, c_deserialize_slice, c_serialize, c_serialize_res, c_size_catch_result};

#[no_mangle]
pub extern "C" fn c_default_payment_address_from_xfvk(
    xfvk: *const c_uchar,
    xfvk_len: size_t,
    xfvk_address_result: *mut *const c_uchar,
) -> size_t {
    c_size_catch_result(|| {
        let xfvk: ExtendedFullViewingKey = unsafe { c_deserialize(xfvk, xfvk_len) }?;
        let xfvk_address = get_xfvk_address(&xfvk, None);

        unsafe { c_serialize_res(xfvk_address, xfvk_address_result) }
    })
}
...
pub extern "C" fn c_payment_address_from_ivk(
    ...
    c_size_catch_result(|| {
        ...
        unsafe { c_serialize_res(address, xfvk_address_result) }
    })
}
...
pub extern "C" fn c_pkd_from_payment_address(
    address: *const c_uchar,
    address_len: size_t,
    pkd_result: *mut *const c_uchar,
) -> size_t {
    c_size_catch_result(|| {
        let address: PaymentAddress = unsafe { c_deserialize(address, address_len) }?;

        unsafe { c_serialize(*address.pk_d(), pkd_result) }
    })
}