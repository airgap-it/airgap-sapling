...
function help () {
  ...
  echo "  -p, --plugin  build using the Gradle plugin (default)"
  echo
}
...
function manual_build () {
  ...
  function build_toolchain () {
    ...
    if [ -d "$dir" ]; then
      ...
      "${ANDROID_NDK}"/build/tools/make_standalone_toolchain.py --api "$SDK_VERSION" --arch "$1" --install-dir "$dir"
...
function plugin_build () {
  ...
  function add_to_localproperties () {
    if ! grep -q "$1" local.properties; then
      echo "$1=$2" >> local.properties
    fi
  }

  echo "  [commands]"
  if which python >/dev/null; then
    echo -e "    \xE2\x9C\x94 python"
  else
    echo -e "    \xE2\x9C\x97 python"

    if which python3 >/dev/null; then
      echo -e "    \xE2\x9C\x94 python3"
      add_to_localproperties "rust.pythonCommand" $(which python3)
    else
      echo -e "    \xE2\x9C\x97 python3"
      
      if which python2 >/dev/null; then
        echo -e "    \xE2\x9C\x94 python2"
        add_to_localproperties "rust.pythonCommand" $(which python2)
      else
        echo -e "    \xE2\x9C\x97 python2"
        ERROR="python could not been found"
      fi
